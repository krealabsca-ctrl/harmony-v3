...
export default function SystemSettingsPage() {
  ...
  const logoMutation = useMutation({
    mutationFn: (file: File) => {
      ...
      return api.post('/admin/system-settings/logo', form, {
        headers: { 'Content-Type': 'multipart/form-data' },
      })
    },
    onSuccess: () => {
    ...
    onError: (err: any) => toast.error(err?.response?.data?.message ?? 'Error al subir logo'),
  })
  ...
  const currentLogo = logoPreviewUrl ?? (data?.logo_url ? data.logo_url + '?t=' + Date.now() : null)
  ...
  return (
    <div className="max-w-2xl mx-auto py-8 px-4 space-y-6">
      ...
      <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 divide-y divide-gray-100 dark:divide-gray-700">
        <div className="px-6 py-4">
          <h2 className="text-base font-semibold text-gray-800 dark:text-gray-100 flex items-center gap-2">
            <span className="w-7 h-7 rounded-lg bg-purple-100 dark:bg-purple-900/40 flex items-center justify-center text-purple-600 dark:text-purple-400 text-sm">🎨</span>
            Logo del sistema
          </h2>
          <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">Se muestra en la pantalla de inicio de sesión y en el sidebar. Recomendado: 200×200 px en PNG o SVG.</p>
        </div>
        <div className="px-6 py-5 space-y-4">

          {/* Preview actual */}
          <div className="flex items-center gap-4">
            <div className="w-20 h-20 rounded-xl border-2 border-dashed border-gray-200 dark:border-gray-600 flex items-center justify-center bg-gray-50 dark:bg-gray-700 overflow-hidden">
              {currentLogo ? (
                <img src={currentLogo} alt="logo" className="w-16 h-16 object-contain" />
              ) : (
            ...
            <div className="flex-1">
              <p className="text-sm font-medium text-gray-700 dark:text-gray-200">
                {selectedLogoFile ? selectedLogoFile.name : data?.logo_url ? 'Logo configurado' : 'Sin logo personalizado'}
              </p>
              <p className="text-xs text-gray-400 dark:text-gray-500 mt-0.5">
                {selectedLogoFile ? `${(selectedLogoFile.size / 1024).toFixed(1)} KB` : 'Formatos: PNG, SVG, JPG'}
              </p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-3">
            ...
            {selectedLogoFile && (
              <button
                onClick={() => logoMutation.mutate(selectedLogoFile)}
