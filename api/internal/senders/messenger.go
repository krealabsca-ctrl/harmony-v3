...
func SendMessengerMedia(ch *models.Channel, to, attachType, localFilePath, mimeType string) (SendResult, error) {
	token, _ := ch.Credentials["access_token"].(string)
	if token == "" {
		return SendResult{}, fmt.Errorf("credenciales Messenger incompletas")
	}
	return sendMetaAttachment(messengerBreaker, "https://graph.facebook.com/v18.0/me/messages", token, to, attachType, localFilePath, mimeType)
}
